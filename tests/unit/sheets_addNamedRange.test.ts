...
describe('sheets_addNamedRange - Unit Tests', () => {
  describe('Schema Validation', () => {
    it('should validate correct parameters', () => {
      ...
      const result = SheetsAddNamedRangeSchema.safeParse(validInput);
    ...
    it('should reject missing name', () => {
      ...
      const result = SheetsAddNamedRangeSchema.safeParse(invalidInput);
    ...
    it('should reject empty name', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.errors[0].message).toBe('Range name is required');
      }
    });
  ...
  describe('API Request Formation', () => {
    it('should form correct AddNamedRangeRequest', () => {
      const input = {
        spreadsheetId: '1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms',
        name: 'DataRange',
        sheetId: 0,
      ...
      expect(expectedRequest.requestBody.requests[0].addNamedRange.namedRange.name).toBe('DataRange');
      expect(expectedRequest.requestBody.requests[0].addNamedRange.namedRange.range.sheetId).toBe(0);
      expect(expectedRequest.requestBody.requests[0].addNamedRange.namedRange.range.startRowIndex).toBe(1);
    });
  });
});
