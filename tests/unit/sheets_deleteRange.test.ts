...
describe('sheets_deleteRange - Unit Tests', () => {
  describe('Schema Validation', () => {
    it('should validate correct parameters for delete range with ROWS shift', () => {
      const validInput = {
      ...
      const result = SheetsDeleteRangeSchema.safeParse(validInput);
    ...
    it('should reject missing spreadsheetId', () => {
      ...
      const result = SheetsDeleteRangeSchema.safeParse(invalidInput);
  ...
  describe('API Request Formation', () => {
    it('should form correct DeleteRangeRequest', () => {
      const input = {
        spreadsheetId: '1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms',
        sheetId: 0,
        startRowIndex: 5,
      ...
      expect(expectedRequest.requestBody.requests[0].deleteRange.range.sheetId).toBe(0);
      expect(expectedRequest.requestBody.requests[0].deleteRange.shiftDimension).toBe('ROWS');
    });
